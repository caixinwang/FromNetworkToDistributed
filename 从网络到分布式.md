...
```
基于磁盘的数据库、基于内存的数据库和缓存数据库是不同类型的数据存储和管理系统，它们在数据存储、访问速度和使用场景等方面有明显的区别。以下是它们之间的主要区别：

数据存储位置：

基于磁盘的数据库：这些数据库将数据存储在永久性磁盘上，通常以文件或表的形式存储，以便长期保存数据，即使系统关闭或重新启动，数据仍然可用。
基于内存的数据库：这些数据库将数据存储在计算机的内存中，因此数据的访问速度非常快，但数据通常不会持久保留，意味着在系统关闭或重新启动后，数据可能会丢失。
缓存数据库：缓存数据库旨在在内存中存储常用的数据，以加速对这些数据的访问。数据通常来自其他持久性存储系统，如磁盘数据库，以及外部数据源。缓存数据库通常不用于永久数据存储，而是用于提高数据访问速度。
访问速度：

基于磁盘的数据库：由于数据存储在磁盘上，访问速度相对较慢，因为需要磁盘读取和写入操作。
基于内存的数据库：由于数据存储在内存中，访问速度非常快，通常可以在微秒或毫秒级别内访问数据。
缓存数据库：与基于内存的数据库类似，访问速度非常快，因为数据也存储在内存中，但数据通常有一定的生命周期，并且需要从其他数据源定期更新。
数据一致性和持久性：

基于磁盘的数据库：通常提供较高的数据持久性和一致性，因为数据被持久保存在磁盘上，即使系统崩溃，数据也不会丢失。
基于内存的数据库：由于数据存储在内存中，数据持久性较低，可能需要额外的手段来确保数据的持久性，如定期将数据写入磁盘。
缓存数据库：数据持久性通常较低，因为它们旨在存储临时数据，而不是永久数据。数据可能会在缓存中过期或被删除。
使用场景：

基于磁盘的数据库：适用于需要长期存储和持久性的数据，如传统的关系数据库管理系统（RDBMS）。
基于内存的数据库：适用于需要极快访问速度的应用，如实时分析、高频交易系统等。
缓存数据库：适用于需要频繁读取相同数据的应用，以减轻对底层数据存储的负载，例如，Web应用中的页面缓存、API调用的结果缓存等。
需要根据特定的应用需求和性能要求选择适当的数据库类型，有时甚至可以将它们结合使用，以满足不同层次的数据管理和访问需求。

基于磁盘的数据库：

MySQL：MySQL是一个流行的开源关系型数据库管理系统（RDBMS），它使用磁盘存储数据并提供持久性、数据一致性和强大的查询功能。
PostgreSQL：PostgreSQL是另一个强大的开源RDBMS，也以其持久性和高级功能而闻名。
基于内存的数据库：

Redis：Redis是一个开源的内存数据库，它专注于高性能、低延迟的数据存储和检索。它通常用作缓存、消息队列和实时分析等应用中的数据存储层。
Memcached：Memcached是另一个基于内存的键值存储系统，旨在加速动态Web应用程序的性能。
缓存数据库：

Redis（再次提到）：除了作为基于内存的数据库，Redis也经常用作缓存数据库，可以将常用数据存储在内存中以提高访问速度。
Apache Kafka：虽然通常被归类为消息队列，但Kafka也可以用作缓存系统，它可以在内存中存储最新的消息以供快速检索，适用于实时数据流处理。
```

```
缓存和内存数据库确实有一些相似之处，因为它们都将数据存储在内存中以提供快速访问，但它们在设计和使用上有重要的区别：

数据持久性：

缓存：缓存主要用于临时存储和快速访问数据，但通常不负责数据的持久性。缓存数据可以随时过期，被删除，或者被替换为新数据，因此不适合用于长期数据存储。
内存数据库：内存数据库旨在将数据持久存储在内存中，但也通常具有将数据定期写入磁盘以确保持久性的机制。这使得内存数据库更适合需要高性能和数据持久性的应用。
数据模型和查询：

缓存：缓存通常以键-值对的形式存储数据，并提供基本的读写操作。它们通常不提供复杂的查询和事务支持。
内存数据库：内存数据库通常提供更丰富的数据模型，允许存储结构化数据，并支持复杂的查询、索引和事务处理。它们更类似于传统的数据库管理系统（DBMS）。
数据更新策略：

缓存：缓存通常使用失效策略，例如LRU（最近最少使用）或TTL（生存时间）来管理数据的更新和淘汰。数据通常来自其他数据源，如数据库或外部API。
内存数据库：内存数据库通常允许直接修改和持久化数据，类似于传统数据库，但更注重性能。
使用场景：

缓存：缓存适用于需要快速访问和减轻底层数据存储的负载的应用，如Web应用中的页面缓存、API调用的结果缓存等。
内存数据库：内存数据库适用于需要高性能、数据持久性和复杂查询的应用，如实时分析、实时报告生成、高频交易系统等。
总的来说，缓存和内存数据库都是用于将数据存储在内存中以提供快速访问的工具，但它们的设计目标和功能不同。选择哪种技术取决于应用程序的需求，如果需要更多的数据持久性、复杂查询和事务支持，那么内存数据库可能更适合。如果只需要简单的数据缓存来提高访问速度，那么缓存可能是更好的选择。
```
...
计算机里边会有内存，这个内存是物理内存，你就想成是一个线性的一个地址空间的字节数组。我们会有程序。程序默认认为整个内存都是自己用的，所以它会有一个虚拟地址，这个red is它其实会有一个虚拟地址空间，需要做一个虚拟地址到真实物理地址的映射。fork相当于子进程的虚拟地址和父进程的虚拟地址都指向同一个物理地址。刚才是不是任意一方修改，另外一方是不是看不到修改。所以那么其实这个时候有一个知识点叫做`copy on wright`，这句话叫做写实复制。子进程并不发生复制，只有在父进程的一方想修改数据的时候才会定向的去复制数据。好处就是创建进程变快了，不可能子进程或者是父进程把所有数据都改一遍。
...
客户端可以只访问这个独立redis，一个瘫痪以后再去连后面的。沿x轴把一个变成多个，那这样的话可以容易解决一台挂掉这种单独故障的问题。随着发展的话，人们发现了，你这个几台机子都支起来了，这个钱别浪费啊。我们的客户端是不是可以把增删改往后面几台去写，读取的操作是不是可以压到后面备用机上去，这就是读写分离。
